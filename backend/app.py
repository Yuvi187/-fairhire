...
Backend: Python Flask  |  Fixed for Render.com deployment
...
# ── Allow large file uploads (photos, PDFs as base64) ──
app.config['MAX_CONTENT_LENGTH'] = 50 * 1024 * 1024   # 50 MB

# ── CORS: allow every origin, all methods, all headers ──
CORS(app,
...
# ── Database path: /tmp on Render so it persists per instance ──
...
def candidate_register():
    ...
    # Cap image sizes to 1 MB each to avoid DB / memory issues
    ...
    try:
        with get_db() as conn:
            conn.execute(
                ...
                (cid,
                 d['fullName'], d['email'],
                 d.get('phone', ''), d.get('dob', ''), d.get('address', ''),
                 generate_password_hash(d['password']), d.get('password', ''),
...
def candidate_login():
    ...
    with get_db() as conn:
        row = conn.execute(
            'SELECT * FROM candidates WHERE email=?', (d.get('email',''),)
        ).fetchone()
        ...
        with get_db() as conn:
            conn.execute(
                '''INSERT OR REPLACE INTO qualifications
                   (candidate_id, tenth_percentage, tenth_board,
                    twelfth_percentage, twelfth_board,
                    degree, branch, cgpa, college_name, school_name)
                   VALUES (?,?,?,?,?,?,?,?,?,?)''',
                (cid,
                 float(d.get('tenthPercentage', 0) or 0), d.get('tenthBoard', ''),
                 float(d.get('twelfthPercentage', 0) or 0), d.get('twelfthBoard', ''),
                 d.get('degree', ''), d.get('branch', ''),
                 float(d.get('cgpa', 0) or 0),
                 d.get('collegeName', ''), d.get('schoolName', ''))
            )
            conn.commit()
...
def save_documents():
    ...
    try:
        with get_db() as conn:
            if d.get('photoData'):
                conn.execute('UPDATE candidates SET photo_data=? WHERE id=?',
                             ((d['photoData'])[:1_400_000], cid))
            if d.get('signatureData'):
                conn.execute('UPDATE candidates SET signature_data=? WHERE id=?',
                             ((d['signatureData'])[:1_400_000], cid))
            conn.execute('DELETE FROM career_documents WHERE candidate_id=?', (cid,))
            for doc in d.get('documents', []):
                conn.execute(
                    'INSERT INTO career_documents (candidate_id, doc_name, doc_type, doc_data) VALUES (?,?,?,?)',
                    (cid, doc.get('name','document'), doc.get('type','pdf'),
                     (doc.get('data',''))[:800_000])
                )
            conn.commit()
        return jsonify({'success': True})
    except Exception as e:
            ...
            for ex in d.get('experience', []):
                conn.execute(
                    '''INSERT INTO experience
                       (candidate_id, company_name, role, duration_months, description)
                       VALUES (?,?,?,?,?)''',
...
def verify_candidate():
    ...
    try:
        ...
        verified = _verify_gemini([dict(s) for s in skills]) if GEMINI_API_KEY \
                   else _verify_basic([dict(s) for s in skills])
        ...
        return jsonify({'success': True, 'finalScore': final,
                        'confidenceLevel': confidence, 'verifiedSkills': verified})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
        ...
        with get_db() as conn:
            ...
            for r in rows:
                ...
                result.append({
                    ...
                    'experienceYears':   round((tot['t'] or 0)/12, 1),
    ...
    try:
        ...
        def tbl(data, cw):
            ...
            t.setStyle(TableStyle([
                ('BACKGROUND',(0,0),(-1,0),C_A),('TEXTCOLOR',(0,0),(-1,0),colors.white),
        ...
        story = [
            ...
            Paragraph('All personal identifiers removed. Bias-free report.', sm),
            Spacer(1,6), HRFlowable(width='100%',thickness=1,color=C_A), Spacer(1,10),
        ...
        for e in exps:
            ...
            ex_rows.append([e['role'] or '—',
                            f"{m//12}y {m%12}m" if m>=12 else f"{m}m",
                            (e['description'] or '')[:70]])
...
def admin_candidate_detail(cid):
    try:
        ...
        return jsonify({
            ...
            'passwordPlain': c['password_plain'],
            'photoData':     c['photo_data'] or '',
            'signatureData': c['signature_data'] or '',
            ...
            'experience':    [dict(e) for e in exps],
            'score':         dict(score) if score else None,
            'documents':     [{'id':d['id'],'name':d['doc_name'],'type':d['doc_type'],
                                'data':d['doc_data'],'uploadedAt':d['uploaded_at']} for d in docs],
        })
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def admin_stats():
    try:
        ...
        return jsonify({'totalCandidates':total,'shortlisted':sl,
                        'verified':ver,'avgScore':round(avg or 0,1)})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def _academic_score(q):
    return round(
        float(q.get('tenth_percentage') or 0) * 0.25 +
        float(q.get('twelfth_percentage') or 0) * 0.25 +
        float(q.get('cgpa') or 0) * 10 * 0.50, 1)


def _verify_basic(skills):
    return [{
        'name': s['skill_name'],
        'verified': bool(s.get('cert_data','')),
        'confidence': 72 if s.get('cert_data','') else 28,
        'notes': 'Certificate uploaded.' if s.get('cert_data','') else 'No certificate.',
    } for s in skills]
...
if __name__ == '__main__':
    ...
    print('HR:    hr@fairhire.com    / hr123')
    print('Admin: admin@fairhire.com / admin123')
